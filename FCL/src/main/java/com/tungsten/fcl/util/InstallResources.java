...
public class InstallResources {
    ...
    private Config safeLoadConfig() {
        try {
            Config parsed = fromJson(readFullyAsString(openAssets(getAppContext(), LAUNCHER_CONFIG)));
    ...
    public void installGameFiles(String oldInstallDir, String srcDir, final SharedPreferences.Editor editor) throws IOException, InterruptedException {
        ...
        install(getAppContext(), getSelectedPath(innerConfig).getAbsolutePath(), srcDir);
        if(editor != null) {
    ...
    public void installConfigFiles() throws Exception {
        try {
            batchDelete(new File(FILES_DIR), new File(CONFIG_DIR), getAppContext().getCacheDir(), getAppContext().getCodeCacheDir());

            for(InitCheckFile file : FileChecker.INSTANCE.getCheckFiles().keySet()) {
                try {
                    Log.d("事件", file.toString());
                    copyAssets(getAppContext(), file.getAssPath(), file.getOutPath());
