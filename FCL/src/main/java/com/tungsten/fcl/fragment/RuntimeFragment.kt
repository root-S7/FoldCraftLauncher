...
class RuntimeFragment : FCLFragment(), View.OnClickListener {
    ...
    private fun initState() {
        try {
            ...
            other = RuntimeUtils.isLatest(FCLPath.FILES_DIR, "/assets/others")
    ...
    private fun install() {
        ...
        bind.apply {
            ...
            if (!other) {
                ...
                Thread {
                    try {
                        RuntimeUtils.copyAssetsFileToLocalDir(context, "others", FCLPath.FILES_DIR)
                        RuntimeUtils.copyAssetsFileToLocalDir(context, "settings", FCLPath.FILES_DIR + "/..")
                        other = true
                    } catch (e: IOException) {
                        e.printStackTrace()
                    }
